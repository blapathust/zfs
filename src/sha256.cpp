...
uint32_t SHA256::majority(uint32_t a, uint32_t b, uint32_t c) { return (a & b) ^ (a & c) ^ (b & c); }
...
void compute_sha256(const void* data, size_t len, uint8_t hash[32]) {
    SHA256 ctx;
    ctx.update((const uint8_t*)data, len);
    uint8_t* digest = ctx.digest();
