...
\begin{lstlisting}[caption={Storage pool abstraction managing multiple virtual devices.}, label={lst:zpool}]
class ZPool {
private:
    std::vector<std::unique_ptr<VDev>> vdevs;
    uint64_t total_capacity;
    uint64_t next_vdev_idx; // Used for round-robin striping

public:
    void add_vdev(std::unique_ptr<VDev> vdev) {
        total_capacity += vdev->get_capacity();
        vdevs.push_back(std::move(vdev));
    }

    // Allocates a block dynamically from the aggregated pool
    uint64_t allocate_block_from_pool() {
        if (vdevs.empty()) return 0;
        
        // Simple round-robin striping allocator
        for (size_t i = 0; i < vdevs.size(); ++i) {
            uint64_t idx = (next_vdev_idx + i) % vdevs.size();
            uint64_t blk = vdevs[idx]->alloc_block();
            if (blk != 0) {
                next_vdev_idx = (idx + 1) % vdevs.size();
                // Return a globally unique block ID (VDev ID + Block Number)
                return (idx << 48) | blk; 
            }
        }
        return 0; // Pool is entirely full
    }
    
    // Routes I/O to the correct physical device
    void read_block(uint64_t global_blk, char* buf) {
        uint64_t vdev_idx = global_blk >> 48;
        uint64_t local_blk = global_blk & 0x0000FFFFFFFFFFFF;
        vdevs[vdev_idx]->read_block(local_blk, buf);
    }
};
\end{lstlisting}

As demonstrated in Listing~\ref{lst:zpool}, the \verb+ZPool+ class abstracts the physical boundaries of individual drives. The \verb+allocate_block_from_pool+ method employs a round-robin strategy to stripe allocations across all attached VDevs, inherently balancing the I/O load. Crucially, the 64-bit block pointers returned by the pool encode both the target VDev identifier (in the upper 16 bits) and the local block number (in the lower 48 bits). This encoding allows upper layers of the file system (such as dnodes and dirents) to reference data globally without needing to understand the underlying physical disk topology. When a new VDev is added via \verb+add_vdev+, its capacity is immediately available for new allocations across all datasets in the pool.
